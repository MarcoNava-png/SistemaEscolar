...
type CandidatoReinscripcion = {
  ...
  estatus: "Elegible" | "Pendiente" | "Bloqueado"; // ejemplo
  adeudo?: number;            // saldo pendiente
  promedio?: number;          // opcional para reglas de reinscripción
};
...
async function fetchReinscripciones(params: {
  ...
  const url = new URL("/api/reinscripciones", typeof window === "undefined" ? "http://localhost" : window.location.origin);
...
function useReinscripciones() {
  ...
  return {
    q, setQ,
    periodoActual, setPeriodoActual,
    periodoNuevo, setPeriodoNuevo,
    estatus, setEstatus,
    page, setPage,
...
export default function Page() {
  ...
  const handleBulkReEnroll = () => {
    ...
    router.push(
      `/dashboard/enrollments/re/nueva?ids=${encodeURIComponent(ids)}&periodoNuevo=${encodeURIComponent(periodoNuevo)}`
    );
  };
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (!Number.isNaN(n)) {
                // reset page si cambias tamaño
                // setPage(1);
                // pero como no usamos page directamente en tabla demo, solo guardamos
              }
            }}
      ...
      <div className="w-full min-w-0 rounded-2xl border border-gray-200/60 bg-white shadow-sm p-4 overflow-visible">
        ...
        <div className="min-w-0 overflow-x-auto">
          <table className="w-full table-auto">
            <colgroup>
              ...
              <col className="w-[120px]" />       {/* Período actual */}
              <col className="w-[120px]" />       {/* Período nuevo */}
              <col className="w-[120px]" />       {/* Estatus */}
              <col className="w-[120px]" />       {/* Adeudo */}
              <col className="w-[110px]" />       {/* Promedio */}
            </colgroup>
              ...
              {(data?.items ?? []).map((r) => {
                ...
                return (
                  <tr key={r.id} className="align-top">
                      ...
                      <div className="flex items-start gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-500">
                            Matrícula: {r.estudiante.matricula ?? "—"}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    {/* Estatus */}
                    <td className="px-3 py-3">
                      <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-medium ring-1 ring-inset ${badgeCls}`}>
                        {r.estatus}
                      </span>
                    </td>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {/* implementa si tu API lo soporta */}}
