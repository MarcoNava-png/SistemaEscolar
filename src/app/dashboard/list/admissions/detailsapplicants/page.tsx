...
import * as inscripcionesService from "@/features/inscripciones/services/inscripcionesService";
import * as admisionesService from "@/features/admisiones/services/admisionesService";
...
function DetailPanel({
  ...
  const guardarContacto = async () => {
    // Persist the contact into local timeline (localStorage) so it appears in the timeline view.
    addEntry({
    ...
    // update local stats (contacts count)
    // if stats were managed externally you could emit an event; here we update local state only
  setContact({ date: "", time: "", channel: "", summary: "", nextAction: "", reminderAt: "", assignedTo: "" });
    setShowContactForm(false);
  };

  // Enroll the aspirante: create enrollment on backend and update admission status
  const [enrolling, setEnrolling] = useState(false);
  const handleEnroll = async () => {
    if (!item.id) return;
    if (!confirm(`¿Confirmas inscribir al aspirante ${item.nombreCompleto ?? item.id}?`)) return;
    setEnrolling(true);
    try {
      await inscripcionesService.createEnrollmentFromAdmission(item);
    } catch (e) {
      // If backend fails, try to still mark admission as enrolled locally
      // and inform the user
      alert("No se pudo crear la inscripcion en el servidor; se marcará localmente.");
    }
    try {
      // mark admission as Inscrito
      await admisionesService.updateAdmission(Number(item.id), { ...item, estatus: "Inscrito" });
    } catch (e) {
      // updateAdmission falls back to local cache; ignore
    }
    setEnrolling(false);
    alert("Aspirante inscrito.");
    // navigate to Inscripciones list
    try {
      router.push("/dashboard/list/Inscripciones");
    } catch {}
  };

  // Watch timeline for reminders while this page is open. If a reminder is due, show an in-page notice.
  useEffect(() => {
    ...
    const check = () => {
      ...
      timeline.forEach((t) => {
        if (t.reminderAt) {
          ...
          if (!isNaN(ts) && ts <= now) {
            // show a lightweight browser alert (could be replaced with toast)
            // mark the reminder as cleared by setting reminderAt to null so it doesn't repeat
            try {
              updateEntry(t.id, { reminderAt: null });
            } catch (e) {}
            // Use a subtle in-page notification by creating a DOM alert — here we'll use alert()
            // In a real app replace with toast library
  ...
  // timeline is loaded by the hook
...
export default function AdmissionsDetailsPage() {
  ...
  const { admissions, error, fetchAdmissions, loading } = useAdmisiones();
  ...
  useEffect(() => {
    ...
    // admissions are AdmisionItem from the API; map to LocalAdmission shape used by this page
    ...
    setSelected(mapped);
    // Aquí podrías calcular stats locales si más tarde guardas contactos/notas en localStorage.
    setStats({ contacts: 0, notes: 0, tasksOpen: 0 });
  }, [selectedId, admissions]);

  // If a selectedId is present but we don't have admissions yet, fetch the first page.
  useEffect(() => {
    if (!selectedId) return;
    if ((admissions == null || admissions.length === 0) && !loading) {
      fetchAdmissions(1, 20).catch(() => {});
    }
  }, [selectedId, admissions, fetchAdmissions, loading]);
  ...
  const clearSelection = useCallback(() => {
    ...
    qs.delete("selected");
    router.replace(
      `/dashboard/list/admissions/detailsapplicants${
