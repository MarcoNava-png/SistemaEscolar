...
export function AdmissionFormModal({
  ...
  // Cuando cambia el estado seleccionado, cargar municipios
  useEffect(() => {
  ...
  // Cuando cambia el municipio seleccionado, cargar asentamientos
  useEffect(() => {
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-x-5 sm:gap-x-6 lg:gap-x-8 gap-y-5 sm:gap-y-6 h-full">
                ...
                <FormField control={form.control} name="idEstadoCivil" render={({ field }) => (
                  <FormItem className="space-y-2">
                    ...
                    <FormControl>
                      <Input type="number" {...field} onChange={(e) => field.onChange(Number(e.target.value))} placeholder="ID de estado civil" className="h-10 rounded-lg text-sm" />
                    </FormControl>
                    <FormMessage className="text-xs" />
                  </FormItem>
                )} />
